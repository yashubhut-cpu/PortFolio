...
export const EducationCertSection: React.FC = () => {
  return (
    <section className="py-16 sm:py-24 bg-[#fefae0] dark:bg-[#1a1a2e] border-t-2 border-black dark:border-[#e85d04] transition-colors duration-300">
            ...
            {educationData.map((edu) => (
              <Card key={edu.id} className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 border-b border-stone-200 dark:border-stone-800 pb-2">
                  ...
                  <span className="font-space-grotesk text-xs font-bold text-stone-600 dark:text-stone-400 flex items-center gap-1">
                    <Calendar size={12} />
                    {edu.year}
                  </span>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {certificationsData.map((cert) => (
                <Card key={cert.id} className="flex items-start justify-between gap-4 p-5">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <span className="font-space-grotesk text-[10px] font-bold bg-[#dde1ff] text-[#0d9488] dark:bg-blue-950 dark:text-blue-200 px-2 py-0.5 border border-black rounded-xs">
                      {cert.date}
                    </span>
                  </div>
                </Card>
